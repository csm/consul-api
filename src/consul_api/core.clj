...
(defmacro blocking-query-params
  [& forms]
  `[{~(symbol "index") :- (field (s/maybe s/Int) {:description "Index to use for consul's blocking queries."}) nil}
    {~(symbol "wait")  :- (field (s/maybe s/Str) {:description "How long to wait for a blocking query"}) nil}
    ~@forms])
